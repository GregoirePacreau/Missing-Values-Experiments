...
def DDC(data):
    ...
    os.system("Rscript R_scripts/DDC.R temp/DDC_data.csv 0.99")
...
def TSGS(data, filter="UBF-DDC", partial_impute=False, tol=1e-4, maxiter=150, method="bisquare", init="emve"):
    ...
    res_mu = pd.read_csv("temp/TSGS_data_res_mu.csv")
    res_S = pd.read_csv("temp/TSGS_data_res_S.csv")
    res_data = pd.read_csv("temp/TSGS_data_res_filtered.csv")
...
def DI(data, initEst="DDCWcov", crit=0.01, maxits=10, quant=0.99, maxCol=0.25):
    ...
    res_mu = pd.read_csv("temp/DI_data_res_mu.csv")
    res_S = pd.read_csv("temp/DI_data_res_S.csv")
